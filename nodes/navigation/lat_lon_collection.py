...
class GPS_Nav(Node):
     """
    Class to handle GPS navigation, calculate angles, and control the movement of a vehicle (e.g., a boat or robot)
    based on the current and target coordinates.
    """
    ...
    def __init__(self):
        ...
        self.lat_lon_cnt = 1         # Latitude/Longitude print counter
        ...
        # TODO: finetune
        self.ANGLE_THR = 8 '''Angle threshold in degrees to determine if turning is needed
        # Create ROS publishers for various topics (navigation input, motor control, etc.)'''
    ...
    def torqeedo_cmd(self, pwrL, pwrR, posL, posR):
        """
        Publishes a command for controlling the motor's power and position.

        Args:
            pwrL (float): Power for the left motor.
            pwrR (float): Power for the right motor.
            posL (float): Position for the left motor.
            posR (float): Position for the right motor.
        """
        msg = String() # Create message
        pub_str = '{"lp":' + str(pwrL) +  ', "rp":' + str(pwrR) + ', "la":' + str(posL) +', "ra":'+str(posR)+ '}'
        msg.data = pub_str # Set the data field of the messag
        self.mc_torqeedo.publish(msg) # Publish the message to control motor

    def lon_callback(self, data):
         """
        Callback function to handle updates for the current longitude.

        Args:
            data (Float64): New longitude data from GPS sensor.
        """
        self.current_lon = data.data #updates current longitude
